...
void pullUsers()
{
    ...
    if(pf){
        while (fscanf(pf,"%s%s%s%s%s%lf",users[totalUser].id,users[totalUser].name,users[totalUser].passwd,users[totalUser].contact,users[totalUser].address,&(users[totalUser].balance))!=EOF) 
        {
            totalUser++;
        }
        fclose(pf);
...
void pushUsers(){
    ...
    {
        fprintf(pf,"%s %s %s %s %s %.lf\n",users[i].id,users[i].name,users[i].passwd,users[i].contact,users[i].address,users[i].balance);
        fclose(pf);
    }
}
